(function () {
  ...
  function scrollToProductForm() {
    ...
    window.setTimeout(() => form.elements.namedItem('name')?.focus(), 250);
  }
  function enhanceProductForm() {
    ...
    if (featuredLabel) {
      ...
      featuredLabel.innerHTML = `<input type="checkbox" name="isFeatured" ${form.elements.namedItem('isFeatured')?.checked ? 'checked' : ''} /><span class="admin-check-copy"><strong>Sản phẩm nổi bật</strong></span>`;
    }
    if (activeLabel) {
      activeLabel.className = 'admin-check-card';
      activeLabel.innerHTML = `<input type="checkbox" name="isActive" ${form.elements.namedItem('isActive')?.checked ? 'checked' : ''} /><span class="admin-check-copy"><strong>Hiển thị trên web</strong></span>`;
    }
  }
